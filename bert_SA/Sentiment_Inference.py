...
class Sentiment_Analysis:
    ...
    def __call__(self, text_list, batch_size=1, threshold=.52):
        ...
        for i in range(n_batches):
            ...
            # return predictions
            tmp = predictions.index(max(predictions))
            pred.append(tmp)
...
if __name__ == '__main__':
    model = Sentiment_Analysis(max_seq_len=800, batch_size=2)

    # https://www.booking.com/reviews.zh-cn.html
    input_path = './corpus/'
    test = pd.read_csv(input_path + 'Test.csv')
    submit = pd.read_csv(input_path+'submit_example.csv')
    test_content = test['content'].astype(str).tolist()
    test_title = test['title'].astype(str).tolist()

    test_content_clean = []
    pattern = re.compile('[a-zA-Z0-9’●►▼!"#$%&\'()*—（+,-./:;<=>?@★、…【】《》？“”‘’[\\]^_`{|}~\s]+')
    for i in range(len(test_content)):
        tmp = pattern.sub('', test_content[i])
        if(len(tmp)<10):
            tmp = pattern.sub('',test_title[i])
        if(len(tmp)==0):
            tmp = "你好"
        test_content_clean.append(tmp[:800])

    # model(test_content_clean)
    pred = model(test_content_clean)
    # print(pred)
    print(len(pred))
    submit['label'] = pred
    submit.to_csv(input_path+'baseline.csv')
