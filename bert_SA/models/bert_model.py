...
# 将输入word转换为词向量并与位置向量相加得到输入的embeding[batch_size,seq_length,embeding_dim]
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        # new_x_shape[batch_size,seq_length,num_heads,heads_size]
        x = x.view(*new_x_shape) # 变化x的维度
        return x.permute(0, 2, 1, 3) # 对维度进行排序
...
# 封装完成输入token[batch_size,seq_length]，使用bert进行计算，得到输出为
# hidden_layers的状态和用于分类的cls的总的状态，还有attention_matrix(可选)
